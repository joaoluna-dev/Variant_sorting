...
df = pd.read_table(tsv_file, sep="\t", header=None)
print(f"  -> Total de variantes carregadas: {len(df)}")

df.iloc[:, CLNSIG_COL] = df.iloc[:, CLNSIG_COL].fillna('Unknown')

print("[2/4] Filtrando variantes por significado clínico...")

pathogenic_df = df[df.iloc[:, CLNSIG_COL] == "Pathogenic"]
...
likely_pathogenic_df = df[df.iloc[:, CLNSIG_COL] == "Likely_pathogenic"]
...
uncertain_df = df[df.iloc[:, CLNSIG_COL] == "Uncertain_significance"]
...
selected.to_csv(txt_file, sep='\t', index=False, header=False)
